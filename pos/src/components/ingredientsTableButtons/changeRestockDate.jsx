 ...
 */
  ...
  const doRestockDate = async e => {
    // e.preventDefault();
    try {
      const body = { restock_date, name };
      // const response = await fetch("https://yifang-backend.onrender.com/inventory/addIngredient", {
      //   method: "POST",
      //   headers: { "Content-Type": "application/json" },
      //   body: JSON.stringify(body)
      // });
      const response = await fetch("http://localhost:5000/inventory/changeRestockDate", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body)
      });
...
export default ChangeRestockDate